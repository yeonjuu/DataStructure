...
public class DoublyLinkedList<T extends Comparable> {
		...
		if(header.next == null)
			...
			while( p != null) {
				if(p.data.compareTo(x) > 0)
					...
					else header.next = p.next;
					if(p.next != null) p.next.prev = p.prev;
					else header.prev = p.prev;
					return true;
				}
				else p = p.next;
			}
	...
	public static void main(String[] args) {
		...
		dll.insert(40); dll.print(); dll.printReverse();
		System.out.println("After inserting 20");
		dll.insert(20); dll.print(); dll.printReverse();
		System.out.println("After inserting 30");
		dll.insert(30); dll.print(); dll.printReverse();
		System.out.println("After inserting 50");
		dll.insert(50); dll.print(); dll.printReverse();
